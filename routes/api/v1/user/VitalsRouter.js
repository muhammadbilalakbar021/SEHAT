...
router.post("/addUserVitals/:id", UserVitalsValidator, async (req, res) => {
  try {
    user_info = await VitalsModel.appendUserVitals(req.params.id, req.body);
    return res.status(200).send(user_info);
  } catch (err) {
...
router.put("/addUserVitals", UserVitalsValidator, async (req, res) => {
  try {
