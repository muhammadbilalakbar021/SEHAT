...
modelSchema.statics.getPatientById = async function(PatientId) {
    // Validating if given user is in our List
    //const patient = patients.find(p => p.id === parseInt(PatientId));
    // If not existing, return 404
    return patient = await PatientModel.findById(PatientId, function(err, patient) {
...
modelSchema.statics.getPatient = async function() {
    var result = []
...
modelSchema.statics.getPage = async function(page = 1, perPage = 10) {
    return this.find().limit(perPage).skip((page - 1) * perPage);
}

modelSchema.statics.validate = async function(RequestedBody) {
    //  Validating
...
modelSchema.methods.addPatient = async function(PatientName) {
    // Add patient
...
modelSchema.virtual("annualSalary").get(function() {
    return this.salary * 12;
});

modelSchema.post("save", async(doc) => {
...
const Model = mongoose.model("Patient", modelSchema);
