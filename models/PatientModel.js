...
patientSchema.statics.removeMedicineStatus = async function(RequestedBody) {
    let patient = await PatientModel.findById(RequestedBody.id);
    let a = [...patient.medicine_status]

    for (let key in a) {
        if (a[key].medicine === RequestedBody.medicine) {
            patient.medicine_status[key].status = false
            break
        }
    }
    console.log(a)
        // patient.medicine_status = a
    patient.save()
}
